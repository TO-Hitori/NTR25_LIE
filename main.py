...
def main(config):
    L.seed_everything(config.lignthing.seed)
    # 回调函数
    early_stop_callback = EarlyStopping(monitor="psnr_val", min_delta=0.00, patience=10, verbose=False, mode="max")
    ckpt_callback = ModelCheckpoint(
        dirpath=mf.smart_path_join("Experinment", config.lignthing.exp_name, "checkpoints"),
    ...
    trainer = L.Trainer(
        max_epochs=config.lignthing.max_epochs,
        min_epochs=config.lignthing.min_epochs,
        log_every_n_steps=1,
        callbacks=callback_list,
        check_val_every_n_epoch=config.lignthing.check_val_every_n_epoch,
        default_root_dir=mf.smart_path_join("Experinment", config.lignthing.exp_name),
