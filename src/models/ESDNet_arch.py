...
class UpSample(nn.Module):
    def __init__(self, dim: int, scale_factor: float = 2, use_conv: bool = True):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        h = F.interpolate(x, scale_factor=self.scale_factor, mode="nearest")
        if self.use_conv:
            h = self.conv(h)
        return h
